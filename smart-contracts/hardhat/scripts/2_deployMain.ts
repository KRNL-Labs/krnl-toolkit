...
export async function deployMainContract(skipHeader = false) {
  ...
  console.log(brandBlue('  ↳ Token Authority Public Address: ') + chalk.white(TAPublicKeyAddress));
  console.log();
  ...
  try {
    ...
    if (error.message.includes("already been verified") || 
        ...
        error.message.includes("Sourcify")) {
      // Just show success message without showing the "already verified" message or URLs
      console.log(brandBlue('✓ ') + chalk.green('CONTRACT VERIFICATION COMPLETE'));
    } else {
  ...
  console.log(`TA Public Address: ${chalk.white(TAPublicKeyAddress.toString())}`);
  console.log(brandBlue('━'.repeat(70)));
  console.log();
